...
class S3Service {
    ...
    async uploadFileWithHashing(file, folder) {
        const fileType = await getFileType(file);
    ...
    async uploadFiles(files, s3Filenames, folder) {
        if (files.length !== s3Filenames.length) {
            throw new Error("Files and s3 filenames must have the same length.");
        }
        const responses = files.map((file, index) => this.uploadFile(file, s3Filenames[index], folder));
        return Promise.all(responses);
    }
    async uploadFilesWithHashing(files, folder) {
        const s3Files = files.map((file) => this.uploadFileWithHashing(file, folder));
        return Promise.all(s3Files);
    }
    async generatePresignedUrl(s3Filename, folder, type, contentDisposition = "inline", expiresIn = 1800) {
    ...
    async removeFile(s3File) {
        const deleteUrl = await this.generatePresignedUrl(s3File.s3Filename, s3File.folder, "DELETE");
        const response = await axios.delete(deleteUrl);
        return response;
    }
    async removeFiles(s3Files) {
        if (s3Files.length === 0)
            return;
