...
class UserController {
    ...
    async predictLeadershipPotential(request, response) {
        const userID = response.locals._id;
        const leadershipPotential = await userService.predictLeadershipPotential(userID);
        response.status(200).send({ leadershipPotential });
    }
    async submitLeadershipReview(request, response) {
        const userID = response.locals._id;
        const reviewer = response.locals.user;
        response
            .status(200)
            .send(await userService.submitLeadershipReview(userID, reviewer.id, request.body));
    }
    catchErrors(handler) {
