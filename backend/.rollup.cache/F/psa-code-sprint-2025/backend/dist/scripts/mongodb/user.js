...
const HASHED_PASSWORD = "$2b$10$TS8eBH1GUf7F3haX1WnX9uqVCzYW9f4ig5abjp4fEMUVkdrqrh91a";
...
export const generateUsers = async (length, includeDefaultUser = true) => {
    const users = await Promise.all(Array.from({ length }).map(async () => {
        return {
            name: faker.person.fullName(),
...
export const generateUsersFromJSON = async (employeeData, includeDefaultUser = true) => {
    const users = await Promise.all(employeeData.map(async (emp) => {
        ...
        const user = {
            ...
            languages: (personal.languages ?? []).map((l) => ({
                name: l.language,
                proficiency: l.proficiency,
            })),
            strengths: (emp.competencies ?? []).map((c) => ({
                name: c.name,
                level: c.level,
            })),
            ...
            careerPath: (emp.positions_history ?? []).map((p) => ({
                name: p.role_title,
...
export async function generateSupervisors() {
    ...
    await Promise.all(users.slice(0, minAssignments).map((user) => {
        user.supervisor = defaultUser._id;
        return user.save();
    }));
    // Assign supervisors to remaining users
    await Promise.all(users.slice(minAssignments).map(async (user) => {
        if (user.supervisor)
            return;
